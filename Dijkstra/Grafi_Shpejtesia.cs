...
namespace Projekti_Dijkstra
{
    class Grafi_Shpejtesia
    {
        ...
        public Grafi_Shpejtesia(double[,] Matrica)
        {
        ...
        public void ShtoNyje(string qyteti)
        {
        ...
        public void Rruga(int nyja_fillestare, int nyja_fundit)
        {
            ...
            while (indeks_Pema < indeks_nyja)
            {
                ...
                for (int i = 0; i < lista.Count; i++)
                {
                    ...
                    if (rruga_Min[lista[i]].distanca != 0)// a ka ndonje rruge qe kalon deri te ajo nyje e i te me e afert
                    {
        ...
        public List<int> GjejiNyjetFqinje(int nyja)
        {
            ...
            for (int i = 0; i < indeks_nyja; i++)
            {
                if (matrica[nyja, i] != infinit && matrica[nyja_aktive, i] != -1 && !nyjet[i].eVizituar)
                {
                    lista.Add(i);
                }
            }
        ...
        public double RrugaMinNgaBurimi(int indeksnyja,int numriiRrugeve)
        {
        ...
        public void Azhurno()
        {
            for (int i = 0; i < indeks_nyja; i++)
            {
                if (!nyjet[i].eVizituar && matrica[nyja_aktive, i] != 0 && distanaca_aktuale + matrica[nyja_aktive, i] > rruga_Min[i].distanca)
                {
        ...
        public int Gjeje_Maksimumin()
        {
            double distanca_minimale = 0;
            int indeksi_min = 0;
            for (int i = 0; i < indeks_nyja; i++)
            {
